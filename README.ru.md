...
## Modern-сборка, профили и очистка
...
Legacy-сборка: сначала установите Python 3.8 x64, затем выполните `scripts\setup_venv_legacy.bat` и `scripts\build_legacy.bat`. Результат: `dist\legacy\ScreenRec.exe`. Legacy использует PySide2/Qt 5 и ImageGrab для захвата окна; Windows Graphics Capture в этом профиле не используется.
...
Затем повторите `scripts\build_modern.bat` и проверьте `build\ScreenRec\warn-ScreenRec.txt`. Скрипт сборки добавляет поставляемые Qt DLL Microsoft runtime в однофайловый пакет и ограничивает PATH системными каталогами Windows и активным `.venv`, чтобы случайная несовместимая Qt/ICU DLL из другой установки не подхватилась.
