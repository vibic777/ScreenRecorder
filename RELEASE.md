# Локальный релиз ScreenRec 0.7.0
...
releases/0.7.0/: ScreenRec.exe, ZIP приложения с папкой browser-extension, отдельный ZIP расширения, ZIP исходников, документация, requirements-windows-lock.txt, manifest.json и SHA256SUMS.txt. Манифест содержит точный коммит. Проверка: Get-FileHash -Algorithm SHA256 <файл>.
...
7. После успешных проверок создать локальный аннотированный тег git tag -a v0.7.0 -m "ScreenRec 0.7.0".
...
Скрипт релиза собирает EXE в build/release-<версия>, затем копирует его в releases/<версия>. Он не заменяет dist/ScreenRec.exe и не останавливает работающую программу. Для проверки новой версии запускайте releases/0.7.0/ScreenRec.exe. Обычный build_windows.bat по-прежнему собирает в dist.
